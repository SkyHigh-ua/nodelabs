...
const selects: {
  ...
  '/task1': "SELECT users.id, users.name, users.avatar_url, photo_url, description, created_at FROM public.users LEFT JOIN public.channels ON users.id = channels.user_id",
  ...
  '/task4': "SELECT channels.id, channels.user_id, description, photo_url, created_at, COUNT(subscriptions.channel_id) AS subscribers FROM public.channels INNER JOIN public.subscriptions ON subscriptions.channel_id = channels.id AND channels.id = '79f6ce8f-ee0c-4ef5-9c36-da06b7f4cb76' GROUP BY channels.id",
  '/task5': "SELECT id, channel_id, title, description, preview_url, file_url, duration, published_at, SUM(CASE WHEN positive THEN 1 ELSE 0 END) AS amount_of_likes FROM public.videos INNER JOIN public.likes ON videos.id = likes.video_id AND videos.published_at >= '2021-09-01'::date GROUP BY videos.id ORDER BY amount_of_likes DESC LIMIT 10",
