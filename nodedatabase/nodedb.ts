...
const selects: {
  ...
  '/task5': "SELECT * FROM (SELECT id, channel_id, title, description, preview_url, file_url, duration, published_at, SUM(CASE WHEN positive THEN 1 ELSE 0 END) AS amount_of_likes FROM public.videos INNER JOIN public.likes ON videos.id = likes.video_id WHERE videos.published_at >= '2021-09-01'::date GROUP BY videos.id ORDER BY amount_of_likes DESC LIMIT 10) AS wrap WHERE amount_of_likes > 4",
